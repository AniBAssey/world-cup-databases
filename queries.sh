...
echo "$($PSQL "SELECT TO_CHAR(AVG(winner_goals + opponent_goals), 'FM9.9900000000000000') FROM games")"
...
echo -e "\nList of unique winning team names in the entire data set:"
echo "$($PSQL "SELECT DISTINCT(name) FROM games JOIN teams ON games.winner_id = teams.team_id ORDER BY name")"

echo -e "\nList of unique winning team names where the opponent scored more than zero goals:"
echo "$($PSQL "SELECT DISTINCT(name) FROM games JOIN teams ON games.winner_id = teams.team_id WHERE opponent_goals > 0 ORDER BY name")"
